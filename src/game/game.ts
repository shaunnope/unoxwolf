...
import { Timer, sleep, Listener } from "~/game/helpers/timer";
...
export class Game implements GameInfo {
    ...
    async run() {
        const joinBtn = new InlineKeyboard()
                            .url(
        ...
        for (let i = 0; i < this.settings.joinTimeout; i++) {
            ...
            if ([10, 30, 60].map(x => this.settings.joinTimeout - x).includes(i)) {
                ...
                if (left > 60) {
                    countdownMsgs.push(
                        this.ctx.reply(
                            this.ctx.t("game_init.minutes_left", { time: Math.floor(left/60) }),
                            {reply_markup: joinBtn}
                        )
                    );
                } else {
                    countdownMsgs.push(
                        this.ctx.reply(
                            this.ctx.t("game_init.seconds_left", { time: left }),
                            {reply_markup: joinBtn}
                        )
                    );
                }
        ...
        for (const msg of countdownMsgs) {
            msg.then(msg => this.ctx.api.deleteMessage(this.chatId, msg.message_id));
        }
